...
set output "result.eps" # Pierwiastki wartosci wlasnych w funkcji parametru alpha
set xlabel "{/Symbol a}"
set ylabel "{/Symbol w}({/Symbol a})"
set key outside horizontal top center
set multiplot
plot for [k=0:5] "result.dat" u 1:(column(k+2)) w lp ls k+1 t "{/Symbol w}_".k.""
...
plot [-5:5][-.1157:.1157] for [k=0:5] "vectors0.dat" i nrSerii u 1:(column(k+2)) w l ls k+1 t "u_".k."(x)"
...
plot [-5:5][-.1157:.1157] for [k=0:6] "vectors100.dat" i nrSerii u 1:(column(k+2)) w l ls k+1 t "u_".k."(x)"
